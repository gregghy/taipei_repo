...
def main():
    ...
    initialize()
    gui = ti.GUI("Taichi MLS-MPM-99", res=1024, background_color=0xFFFFFF)
    frame = 0
    max_frames = 500
    while not gui.get_event(ti.GUI.ESCAPE, ti.GUI.EXIT) and frame < max_frames:
        for s in range(int(2e-3 // dt)):
            substep()
        gui.circles(
            x.to_numpy(),
            radius=3.0,
            palette=[0x068587, 0xFFFF00, 0xFF0000],
            palette_indices=material,
        )
        gui.show(f"{output_dir}/{frame:06d}.png")
        frame += 1
